...
@jax.jit
def kl_divergence(mean: jnp.ndarray, logvar: jnp.ndarray, min_logvar: float = -6.0, max_logvar: float = 6.0) -> jnp.ndarray:
    """
    ...
    # Clamp logvar to prevent exp from exploding or underflowing
    logvar = jnp.clip(logvar, a_min=min_logvar, a_max=max_logvar)
